...
class Task_e_determine_pixel_sensitivity:
    ...
    def determine_pixel_sensitivity(flat, white_spectrum, white_wavelength, order_centers, mean_dark):
        ...
        for i, center in enumerate(order_centers):
            ...
            half_width = 20  # Adjust based on order width
    ...
    #         order_centers: Centers of the spectral orders
    #         wavelength_solutions: List of (order_idx, coefficients, rms_error) for each order
    #         white_wavelength: Wavelength array for the white lamp spectrum
    #         white_spectrum: Intensity array for the white lamp spectrum
        
    #     Returns:
    #         sensitivity_map: 2D array of the same shape as flat with relative sensitivity values
    #     """
    ...
    #     for order_idx, coeffs, _ in wavelength_solutions:
    #         # Find the corresponding order center
    #         try:
    #             center_index = np.where(order_centers == order_idx)[0][0]
    #             center = order_centers[center_index]
    #         except:
    #             # If the order index doesn't match directly, find the closest one
    #             center_index = np.argmin(np.abs(order_centers - order_idx))
    #             center = order_centers[center_index]
    ...
    #     # Normalize the sensitivity map to have a maximum of 1
    #     if np.max(sensitivity_map) > 0:
    #         sensitivity_map = sensitivity_map / np.max(sensitivity_map)
    ...
    #     # Sample points across orders and create a scatter plot
    #     wavelengths = []
    #     sensitivities = []
        
    #     # Sample points from the sensitivity map
    #     for order_idx, coeffs, _ in wavelength_solutions:
    #         # Find corresponding order center
    #         try:
    #             center_index = np.where(order_centers == order_idx)[0][0]
    #             center = order_centers[center_index]
    #         except:
